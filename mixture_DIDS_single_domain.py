...
def hierarchical_clustering(data_dict, initial_clusters=64, target_dim=1024, sample_dim=2000000,
...
def train_model(json_path, model_path, output_path, dataset_alias, round_number=0, lr=2e-6, min_lr=0):
    ...
    config_yaml_path = f"{DATA_PATH}/yaml/training_config_{dataset_alias}.yaml"
...
def main(args, single_domain_dataset_name):
    set_seed(42)
    ...
    logging.info(f"we only use {single_domain_dataset_name} as eval dataset")
    eval_datasets = {single_domain_dataset_name: eval_datasets[single_domain_dataset_name]}
    # 固定采样用于验证集loss计算
    fixed_eval_samples = {}
    ...
    utils.run_lm_eval_single_domain(args.output_model_path, lora_path=None, single_domain_dataset_name=single_domain_dataset_name)
...
if __name__ == "__main__":
    ...
    dataset_configs = ["mmlu", "truthfulqa", "bbh", "gsm8k", "mathqa","arc_easy","boolq","ifeval","logiqa","minerva_math","piqa","pubmedqa"]
    for dataset_name in dataset_configs:
        args.dataset_alias = f"{args.task}_{args.total_samples // 1000}k_{args.round_number}round_{args.num_clusters}clusters_single_domain_{dataset_name}"
        args.output_model_path = f"{output_model_path}/{args.dataset_alias}_{args.pretrained.replace('/', '-')}"
