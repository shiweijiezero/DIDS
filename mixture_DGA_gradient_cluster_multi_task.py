        ...
        for conv in batch_convs:
            ...
            if random_matrix is None:
                random_matrix = torch.randn(sample_dim, target_dim,
                                            ...
                                            device=device)
                logging.info(
                    f"Initialized Random Projection matrix with shape: {random_matrix.shape} and dtype: {random_matrix.dtype}")
...
def hierarchical_clustering(data_dict, initial_clusters=64, target_dim=384, sample_dim=10000,
    ...
    # 检查缓存
    cache_path = Path(
        cache_dir) / f"gradient_cluster_results_{initial_clusters}_{target_dim}d_{sample_dim}s_{model_name.replace('/', '_')}_mlp.pkl"
    if cache_path.exists():
...
def main(args):
    ...
    for round_number in range(args.round_number):
        ...
        if round_number == 0:
            ...
            mixture_dic = utils.DGA_mixture_multi_task(train_data_dic, eval_datasets, round_samples, args.num_clusters,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default="mixture_DGA_gradient_cluster_multi_task")
