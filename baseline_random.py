...
def main(args):
    ...
    if os.path.exists(f"data/json/{dataset_alias}.jsonl"):
        train_json_path = f"json/{dataset_alias}.jsonl"
    else:
        logging.info("loading data from scratch")
        # dic = load_specific(dataname="science", dic=dic)
        # dic = load_specific(dataname="OpenOrca", dic=dic)
        # dic = load_specific(dataname="tulu", dic=dic)
        ...
        print(f"train_data_dic的类型有{list(train_data_dic.keys())}\n共计{len(train_data_dic)}个")
        training_data, train_json_path = generate_training_data(train_data_dic, total_samples, args.dataset_alias)
    ...
    utils.run_lm_eval(output_model_path, lora_path=None)
